...
public class PlayerMovement : MonoBehaviour
{
    ...
    void Update()
    {
        //Assigns a new Vector3 to the objects transform that will get assigned a new value and change the objects position using it
        ...
        //Assigns newPos back to the objects transform so that it can use the updated values given to newPos to change the object 
        transform.position = newPos;
    }
    //Creates a function with an argument that gives us access to the values given by the input system
    public void OnMove(InputAction.CallbackContext context)
    {
        {
                //The player movement Vector3 gets assigned the values from the input system as a Vector2 so that player inputs can uses those values to move the object
